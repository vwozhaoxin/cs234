...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        self.s = tf.placeholder(tf.uint8,[None,state_shape[0],state_shape[1],state_shape[2]*self.config.state_history],name ='state')
        self.a = tf.placeholder(tf.int32,[None],name='action')
        self.r = tf.placeholder(tf.float32,[None],name='reward')
        self.sp = tf.placeholder(tf.uint8,[None,state_shape[0],state_shape[1],state_shape[2]*self.config.state_history],name='next_state')
        self.done_mask = tf.placeholder(tf.bool,shape=[None],name='mask')
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        with tf.variable_scope(scope,reuse = reuse):
            #flatten_layer = tf.layers.flatten(state)
            #out = tf.layers.dense(flatten_layer,num_actions)
            input_layer = layers.flatten(state,scope=scope)
            #input_layer = layers.bias_add(input_layer)
            out = layers.fully_connected(input_layer,num_actions,reuse=reuse,scope =scope,activation_fn=None)
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        variables_t= tf.get_collection(key=tf.GraphKeys.TRAINABLE_VARIABLES,scope=target_q_scope) 
        variables_q= tf.get_collection(key=tf.GraphKeys.TRAINABLE_VARIABLES,scope=q_scope) 
        op = ['']*len(variables_t)
        for i,var in enumerate(variables_q):
            op[i]=tf.assign(variables_t[i],var)

        #var = tf.get_collection(key=tf.GraphKeys.TRAINABLE_VARIABLES,scope=target_q_scope) 
        #op_1=tf.assign(var,variables) 
        #op_2=tf.add_to_collection(q_scope,var) 
    ...
    def add_loss_op(self, q, target_q):
        ...
        mask_done = tf.cast(self.done_mask,tf.float32)
        Q_samp = self.r +tf.multiply(self.config.gamma*tf.reduce_max(target_q,1),1-mask_done)
        actions = tf.one_hot(self.a,num_actions)
        Q = tf.reduce_sum(tf.multiply(actions,q),axis=1)
        self.loss = tf.reduce_mean(tf.square(Q_samp-Q))
        ##############################################################
        ######################## END YOUR CODE #######################
    ...
    def add_optimizer_op(self, scope):
        ...
        gradients_and_vars = optimizer.compute_gradients(self.loss,variables)
        gradients = [i[0] for i in gradients_and_vars]
        var = [i[1] for i in gradients_and_vars]  
        #print(gradients_and_vars)         
        if self.config.grad_clip==True:
            #print(gradients[0].shape,gradients[1].shape,gradients[2].shape)
            #print(var[0].shape,var[1].shape,var[2].shape)
            tmp =[]
            for gradient in gradients:
                gradient = tf.clip_by_norm(gradient,self.config.clip_val)
                tmp.append(gradient)
            gradients = tmp
            gradients_and_vars = [ (gradients[i],var[i]) for i in range(len(var))]
        self.train_op = optimizer.apply_gradients(gradients_and_vars)
