...
def build_mlp(
  ...
  with tf.variable_scope(scope):
    for i in range(n_layers):
      hidden_layer = tf.layers.dense(hidden_layer,units=size,activation=tf.nn.relu)
    out = tf.layers.dense(hidden_layer,units=output_size,activation=output_activation)
  return out# 
  #######################################################
  #########          END YOUR CODE.          ############
...
class PG(object):
  ...
  def add_placeholders_op(self):
    ...
    self.observation_placeholder = tf.placeholder(tf.float32,[None,self.observation_dim])  # TODO
    if self.discrete:
  ...
  def build_policy_network_op(self, scope = "policy_network"):
    ...
    if self.discrete:
      with tf.variable_scope(scope):
        action_logits = build_mlp(self.observation_placeholder,self.action_dim,scope,self.config.n_layers,self.config.layer_size)        # TODO 
      
        self.sampled_action =tf.squeeze(tf.multinomial(action_logits,1),1)
        print(self.sampled_action.shape)  # TODO 
        self.logprob = -tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.action_placeholder,logits=action_logits)         # TODO 
    else:
      with tf.variable_scope(scope):
        action_means = build_mlp(self.observation_placeholder,self.action_dim,scope,self.config.n_layers,self.config.layer_size)        # TODO 
        log_std = tf.get_variable('log_std',shape=[self.action_dim],dtype=tf.float32)              # TODO 
        self.sampled_action = tf.random_normal(tf.shape(action_means),mean=action_means,stddev=tf.exp(log_std) )  # TODO 
  ...
  def add_loss_op(self):
    ...
    self.loss =tf.reduce_mean(tf.multiply(-self.logprob,self.advantage_placeholder)) # TODO
    #######################################################
    #########          END YOUR CODE.          ############
  ...
  def add_baseline_op(self, scope = "baseline"):
    ...
    self.baseline = tf.squeeze(build_mlp(self.observation_placeholder,1,scope,self.config.n_layers,self.config.layer_size))# TODO
    self.baseline_target_placeholder = tf.placeholder(tf.float32,shape=[None]) # TODO
    self.update_baseline_op = tf.train.AdamOptimizer(self.lr).minimize(tf.losses.mean_squared_error(self.baseline,self.baseline_target_placeholder)) # TODO
    #######################################################
    #########          END YOUR CODE.          ############
  ...
  def get_returns(self, paths):
    ...
    for path in paths:
      ...
      #########   YOUR CODE HERE - 5-10 lines.   ############
      steps = len(rewards)
      path_returns = [np.sum(rewards[i:]*np.power(self.config.gamma,np.arange(steps-i)))for i in range(len(rewards)) ]# TODO
  ...
  def calculate_advantage(self, returns, observations):
    ...
    #########   YOUR CODE HERE - 5-10 lines.   ############
    if self.config.use_baseline:
      bn =self.sess.run(self.baseline,feed_dict={self.observation_placeholder:observations})
      #bn = (bn-np.mean(bn,0))/np.std(bn,0)
      adv= returns- bn   
    if self.config.normalize_advantage:
      adv = (adv-np.mean(adv,0))/np.std(adv,0)
    #######################################################
    #########          END YOUR CODE.          ############
    return adv
  ...
  def update_baseline(self, returns, observations):
    ...
    #norm_returns = (returns-np.mean(returns,0))/np.std(returns,0)
